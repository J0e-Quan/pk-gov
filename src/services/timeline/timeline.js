...
async function getData(currentDate, endDate) {
  try {
    const response = await fetch(
      'https://mycal-api.huijun00100101.workers.dev/v1/holidays/between?start=' +
        currentDate +
        '&end=' +
        endDate +
        '&state=penang'
...
function showNextHoliday() {
  ...
  if (data.length === 0) {
    ...
    holidayDate.textContent =
      'Timeline will be updated with new information on the first day of ' +
